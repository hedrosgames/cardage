...
using UnityEngine;
[CreateAssetMenu(menuName="Game/Rules/Plus")]
public class RulePlus : SOCapture
{
    public override void OnCardPlayed(CardSlot origin, SOCardData card, int ownerId, ManagerBoard board, List<CardSlot> captured)
    {
        ...
        TrySum(origin, card.top, board.GetSlot(origin.gridPosition.x, origin.gridPosition.y + 1), sums, s => s.currentCardView.cardData.bottom);
        TrySum(origin, card.right, board.GetSlot(origin.gridPosition.x + 1, origin.gridPosition.y), sums, s => s.currentCardView.cardData.left);
        TrySum(origin, card.bottom, board.GetSlot(origin.gridPosition.x, origin.gridPosition.y - 1), sums, s => s.currentCardView.cardData.top);
        TrySum(origin, card.left, board.GetSlot(origin.gridPosition.x - 1, origin.gridPosition.y), sums, s => s.currentCardView.cardData.right);
    ...
    private void TrySum(CardSlot origin, int placedVal, CardSlot neigh, Dictionary<CardSlot, int> sums,
    System.Func<CardSlot, int> getNeighVal)
    {
