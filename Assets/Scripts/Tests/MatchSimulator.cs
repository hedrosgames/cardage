...
/// <summary>
/// Simulador completo de partidas para testar todas as regras em condições reais
/// </summary>
public class MatchSimulator : MonoBehaviour
{
    [Header("Configuração")]
    [Tooltip("Número de partidas por regra (reduza para testes mais rápidos)")]
    public int matchesPerRule = 100;
    [Tooltip("Executar automaticamente ao iniciar")]
    public bool runOnStart = false;
    [Tooltip("Mostrar progresso detalhado")]
    ...
    public void RunAllSimulations()
    {
        ...
        Debug.Log($"=== INICIANDO SIMULAÇÃO DE PARTIDAS ===");
        Debug.Log($"Configuração: {matchesPerRule} partidas por regra");
        Debug.Log($"Isso pode demorar alguns minutos...");
        
        // Lista de todas as regras para testar
        ...
        // Testar regras de vitória separadamente
        ...
        Debug.Log($"\n=== SIMULAÇÃO CONCLUÍDA ===");
        Debug.Log($"Tempo total: {elapsedTime:F2} segundos ({elapsedTime / 60f:F2} minutos)");
        GenerateReport();
    }
    ...
    private void SimulateRule(string ruleName, System.Type ruleType, int matches)
    {
        ...
        for (int i = 0; i < matches; i++)
        {
            try
            {
                ...
                // Mostrar progresso mais frequente para feedback melhor
                if (showDetailedProgress)
                {
                    int progressInterval = matches > 100 ? 50 : (matches > 10 ? 10 : 1);
                    if (i % progressInterval == 0 && i > 0)
                    {
                        Debug.Log($"  [{ruleName}] Progresso: {i}/{matches} partidas ({i * 100f / matches:F1}%)");
                    }
                }
            }
        ...
        Debug.Log($"[{ruleName}] Finalizado: {stats.activations} ativações em {matches} partidas (média: {stats.averageActivationsPerMatch:F2} por partida)");
    }
    ...
    private void SimulateVictoryRule(string ruleName, System.Type ruleType, int matches)
    {
        ...
        Debug.Log($"[{ruleName}] Finalizado: {stats.activations} ativações em {matches} partidas");
    }
    ...
    {
        // Criar ambiente de simulação (sem visual, mais rápido)
        ...
        // Simular partida completa (9 turnos, uma carta por turno)
        // Otimizado: não criar GameObjects desnecessários
        for (int turn = 0; turn < 9; turn++)
        {
            // Determinar qual jogador joga (alternado)
            int currentPlayer = turn % 2 == 0 ? ManagerGame.ID_PLAYER : ManagerGame.ID_OPPONENT;
            
            // Escolher slot aleatório vazio
            var emptySlots = simEnv.board.GetAllSlots().Where(s => !s.IsOccupied).ToList();
            if (emptySlots.Count == 0) break; // Tabuleiro cheio
            ...
            // Colocar carta no slot (simulado, sem visual)
            PlaceCardSimulated(targetSlot, card, currentPlayer);
            
            // Simular jogada (dispara eventos de captura)
            List<CardSlot> captured = new List<CardSlot>();
            
            // Verificar se a regra está ativa e contar ativações
            foreach (var rule in simEnv.activeRules)
            {
                if (rule.GetType() == ruleType)
                {
                    ...
                    // Verificar se a regra funcionou corretamente
                    try
                    {
                        // Executar a regra
                        rule.OnCardPlayed(targetSlot, card, currentPlayer, simEnv.board, captured);
                        
                        // Se chegou aqui sem exceção, a regra funcionou corretamente
                        // Regras de captura podem não capturar em alguns turnos (isso é normal)
                        // Regras de efeito/especiais não fazem captura, então sempre sucesso se não lançou exceção
                ...
                {
                    // Aplicar outras regras normalmente
                    rule.OnCardPlayed(targetSlot, card, currentPlayer, simEnv.board, captured);
                }
            }
    ...
    private (int activations, int successfulActivations, int failedActivations) SimulateVictoryMatch(System.Type ruleType)
    {
        ...
        // Preencher tabuleiro completamente
        ...
        // Vitória sempre ativa uma vez por partida
    ...
    private SimulationEnvironment CreateSimulationEnvironment(System.Type primaryRuleType)
    {
        ...
        // Criar tabuleiro (sem visual, mais rápido)
        GameObject boardGO = new GameObject("SimBoard");
        boardGO.hideFlags = HideFlags.HideAndDontSave; // Não salvar na hierarquia
    ...
    {
        // Criar CardView simulado sem prefab (otimizado)
        GameObject cardGO = new GameObject("SimCard");
        cardGO.hideFlags = HideFlags.HideAndDontSave; // Não salvar na hierarquia
    ...
    private SOCardData CreateRandomCard()
    {
        ...
        // Adicionar regras gerais aleatórias (30% de chance)
    ...
    private void GenerateReport()
    {
        ...
        Debug.Log($"Relatório salvo em: {path}");
        #endif
    }
}
