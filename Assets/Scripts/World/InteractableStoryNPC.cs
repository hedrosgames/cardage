...
public class InteractableStoryNPC : Interactable, INotificationProvider
{
    ...
    // NOVO: Controle explícito de fase para evitar confusão de diálogos iguais
    ...
    private void TriggerSequence()
    {
        ...
        Debug.Log($"[NPC {gameObject.name}] 1. JOGADOR DETECTADO! Iniciando sequência.");

        hasTriggered = true;
        isClosingPhase = false; // Reset da fase
        ShowAlert();
    ...
    private IEnumerator StartSequenceAfterNotification()
    {
        ...
        Debug.Log($"[NPC {gameObject.name}] 2. Movimento agendado.");
    ...
    private IEnumerator DelayedMoveToPointB()
    {
        ...
        if (pointB != null)
        {
            Debug.Log($"[NPC {gameObject.name}] 3. Andando para o Ponto B.");
            yield return StartCoroutine(MoveToPoint(pointB.position, OnReachedPointB));
        }
        else
        {
            Debug.Log($"[NPC {gameObject.name}] 3. Ponto B nulo. Pulando para diálogo.");
    ...
    {
        Debug.Log($"[NPC {gameObject.name}] 4. Chegou no Ponto B.");
    ...
    private void StartDialogue()
    {
        if (dialogue != null)
        {
            Debug.Log($"[NPC {gameObject.name}] 5. Iniciando Diálogo de Abertura.");
            isWaitingForDialogue = true;
            activeDialogue = dialogue;
            isClosingPhase = false; // Garante que estamos na fase 1
            GameEvents.OnRequestDialogue?.Invoke(dialogue);
        }
        else
        {
            Debug.LogWarning($"[NPC {gameObject.name}] Diálogo de Abertura é NULO. Pulando.");
    ...
    {
        Debug.Log($"[NPC {gameObject.name}] EVENTO DE DIÁLOGO RECEBIDO. Seq: {sequence?.name}. Fase Fechamento: {isClosingPhase}");

        if (!isWaitingForDialogue) return;

        // Se sequence for diferente do ativo (e não nulo), ignora
        if (sequence != null && sequence != activeDialogue && activeDialogue != null) return;

        isWaitingForDialogue = false;
        activeDialogue = null;

        // CORREÇÃO: Usa a flag de estado em vez de comparar o objeto de diálogo
        if (isClosingPhase)
        {
            Debug.Log($"[NPC {gameObject.name}] 8. Diálogo de Fechamento finalizado.");
            OnDialogueClosingFinished();
        }
        else
        {
            Debug.Log($"[NPC {gameObject.name}] 6. Diálogo de Abertura finalizado.");
    ...
    private void OnDialogueOpeningFinished()
    {
        ...
        if (pointC != null)
        {
            Debug.Log($"[NPC {gameObject.name}] 7. Indo para Ponto C.");
            StartCoroutine(MoveToPoint(pointC.position, OnReachedPointC));
        }
        else
        {
            Debug.Log($"[NPC {gameObject.name}] 7. Sem Ponto C. Iniciando fechamento.");
    ...
    private void StartClosingDialogue()
    {
        ...
        {
            Debug.Log($"[NPC {gameObject.name}] 8. Iniciando Diálogo de Fechamento.");
            isWaitingForDialogue = true;
            activeDialogue = dialogueClosing;
            isClosingPhase = true; // MARCA QUE AGORA ESTAMOS NO FECHAMENTO
            GameEvents.OnRequestDialogue?.Invoke(dialogueClosing);
        }
        else
        {
            Debug.Log($"[NPC {gameObject.name}] Sem Diálogo de Fechamento. Finalizando direto.");
            isClosingPhase = true; // Necessário para lógica consistente
            OnDialogueClosingFinished();
        }
    }
    ...
    {
        Debug.Log($"[NPC {gameObject.name}] 9. SEQUÊNCIA COMPLETA. Liberando jogador e destruindo.");
        ReleasePlayer();
    ...
    private IEnumerator ExecuteEventsAndDestroy()
    {
        ...
        Debug.Log($"[NPC {gameObject.name}] Executando UnityEvents...");
        try { onDialogueClosingFinished?.Invoke(); }
        catch (System.Exception e) { Debug.LogError($"Erro no UnityEvent: {e.Message}"); }

        if (destroyOnFinish)
        {
            Debug.Log($"[NPC {gameObject.name}] DESTRUINDO OBJETO AGORA.");
            Destroy(gameObject);
        }
    }
    ...
    {
        hasTriggered = false; // Destrava o Update
    ...
    private IEnumerator MoveToPoint(Vector3 targetPosition, System.Action onArrival)
    {
        ...
        while (Vector3.Distance(transform.position, targetPosition) > arrivalDistance)
        {
            ...
            if (timer > timeOut)
            {
                Debug.LogWarning($"[NPC {gameObject.name}] TIME OUT no movimento. Forçando chegada.");
                break;
            }
    ...
    {
        Debug.Log($"[InteractableStoryNPC] ForceDestroyNPC chamado para {gameObject.name}");
        Destroy(gameObject);
    }
}